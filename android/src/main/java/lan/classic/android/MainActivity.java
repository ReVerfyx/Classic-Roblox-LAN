...
public final class MainActivity extends Activity implements GameView.Session {
    ...
    protected void onActivityResult(int requestCode,int resultCode,Intent data){
        ...
        new Thread(new Runnable(){public void run(){File dst=null;try{
            File dir=new File(getFilesDir(),"maps");if(!dir.isDirectory()&&!dir.mkdirs())throw new IOException("Cannot create maps folder");
            dst=File.createTempFile("place_",".rbxlx",dir);
            try(InputStream in=getContentResolver().openInputStream(uri);OutputStream out=new FileOutputStream(dst)){
                if(in==null)throw new IOException("Cannot open selected file");byte[] buf=new byte[8192];int n;while((n=in.read(buf))!=-1)out.write(buf,0,n);
            }
