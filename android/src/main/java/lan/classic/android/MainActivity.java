...
public final class MainActivity extends Activity implements GameView.Session {
    ...
    private void home() {
        ...
        setContentView(new ClassicHome(this,user,language.equals("ru"),settings.getBoolean(user+".recent",false),settings.getBoolean(user+".favorite",false),friends(),settings.getString(user+".chat",""),new ClassicHome.Actions() {
            public void play(){placeName="Classic Baseplate";importedMapPath="";settings.edit().remove("importedMapPath").apply();start(false,null,Net.PORT);}
            public void playMode(String place){placeName=place;importedMapPath=""; if(place.equals("Rocket Arena"))mode=GameConfig.Mode.ROCKET_ARENA;else if(place.equals("Sword Fight on the Heights"))mode=GameConfig.Mode.SWORD_FIGHT;else if(place.equals("Natural Disaster Survival"))mode=GameConfig.Mode.DISASTERS;else mode=GameConfig.Mode.SANDBOX;settings.edit().remove("importedMapPath").putString("place",placeName).putString("mode",mode.name()).apply();start(false,null,Net.PORT);}
            public void host(){start(true,null,Net.PORT);}
            public void servers(){MainActivity.this.servers();}
            public void settings(){preferences();}
            public void avatar(){avatarEditor();}
    ...
    private void preferences(){final LinearLayout root=page(t("Settings","Настройки"));root.addView(label("Language / Язык",14));final String[] codes={"en","ru","de","es","fr","pt","pl"};Spinner langs=new Spinner(this);langs.setAdapter(new ArrayAdapter<String>(this,android.R.layout.simple_spinner_dropdown_item,new String[]{"English","Русский","Deutsch","Español","Français","Português","Polski"}));langs.setSelection(Arrays.asList(codes).indexOf(language));root.addView(langs);langs.setOnItemSelectedListener(new android.widget.AdapterView.OnItemSelectedListener(){public void onNothingSelected(AdapterView<?> a){}public void onItemSelected(AdapterView<?> a,View v,int p,long id){language=codes[p];settings.edit().putString("language",language).putString(user+".language",language).apply();}});
        ...
        addButton(root,t("Load your RBXLX place","Загрузить свою карту RBXLX"),new View.OnClickListener(){public void onClick(View v){Intent pick=new Intent(Intent.ACTION_GET_CONTENT);pick.setType("application/xml");pick.addCategory(Intent.CATEGORY_OPENABLE);startActivityForResult(Intent.createChooser(pick,t("Choose RBXLX place","Выберите карту RBXLX")),PICK_RBXLX);}});
    ...
    protected void onActivityResult(int requestCode,int resultCode,Intent data){super.onActivityResult(requestCode,resultCode,data);if(requestCode!=PICK_RBXLX||resultCode!=RESULT_OK||data==null||data.getData()==null)return;final Uri uri=data.getData();new Thread(new Runnable(){public void run(){String error=null;try{File dir=getExternalFilesDir("maps");if(dir==null)dir=new File(getFilesDir(),"maps");if(!dir.exists()&&!dir.mkdirs())throw new IOException("Cannot create maps folder");String name="place_"+System.currentTimeMillis()+".rbxlx";File dst=new File(dir,name);InputStream in=getContentResolver().openInputStream(uri);if(in==null)throw new IOException("Cannot open selected file");OutputStream out=new FileOutputStream(dst);byte[] buf=new byte[8192];int n,total=0;while((n=in.read(buf))!=-1){total+=n;if(total>2*1024*1024)throw new IOException("RBXLX exceeds 2 MiB import budget");out.write(buf,0,n);}in.close();out.close();importedMapPath=dst.getAbsolutePath();placeName=dst.getName();settings.edit().putString("importedMapPath",importedMapPath).putString("place",placeName).apply();}catch(Exception e){error=e.toString();}final String result=error;handler.post(new Runnable(){public void run(){Toast.makeText(MainActivity.this,result==null?t("RBXLX saved in app data/maps","RBXLX сохранён в data/maps приложения"):result,Toast.LENGTH_LONG).show();}});}},"rbxlx-copy").start();}
