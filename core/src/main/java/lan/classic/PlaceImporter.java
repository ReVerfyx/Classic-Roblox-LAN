...
public final class PlaceImporter {
    public static Place read(InputStream stream)throws Exception {
        ...
        while((n=stream.read(buf))!=-1){if(bytes.size()+n>2*1024*1024)throw new IOException("Map exceeds 2 MiB import budget");bytes.write(buf,0,n);}
        ...
        parser.parse(new ByteArrayInputStream(bytes.toByteArray()),new DefaultHandler(){
            ...
            public void startElement(String uri,String local,String name,Attributes attrs)throws SAXException{
                if(++depth>64)throw new SAXException("XML nesting budget");text.setLength(0);leaf=name;
                if(name.equals("Item")){if(stack.size()>32)throw new SAXException("Item nesting budget");stack.add(new Entry(attrs.getValue("class")));}
                String p=attrs.getValue("name");if(p!=null){property=p;values.clear();}
            }
            public void characters(char[] ch,int start,int len)throws SAXException{if(text.length()+len>4096)throw new SAXException("Property too long");text.append(ch,start,len);}
            public void endElement(String uri,String local,String name)throws SAXException{
                depth--;if(name.equals("Item")){Entry e=stack.remove(stack.size()-1);if(e.part!=null){if(out.parts.size()>=512)throw new SAXException("Map exceeds 512 Parts");e.part.originX=e.part.x;e.part.originY=e.part.y;e.part.originZ=e.part.z;out.parts.add(e.part);}if(e.source!=null){if(out.scripts.size()>=32)throw new SAXException("Map exceeds 32 scripts");out.scripts.add(e.source);out.scriptNames.add(e.name==null?e.className:e.name);}return;}
                if(stack.isEmpty())return;Entry current=stack.get(stack.size()-1);String s=text.toString().trim();
                if(name.equals("string")&&property.equals("Source")&&current.isScript()){current.source=s;return;}
                if(name.equals("string")&&property.equals("Name")){current.name=s;return;}
                if(current.part==null)return;Part p=current.part;
                try{
                    if(name.equals("X")||name.equals("Y")||name.equals("Z")||name.matches("R[012][012]")||name.equals("R")||name.equals("G")||name.equals("B")){float v=Float.parseFloat(s);if(Float.isNaN(v)||Float.isInfinite(v)||Math.abs(v)>100000)throw new NumberFormatException();values.put(name,v);}
                    if(name.equals("Vector3")&&property.equalsIgnoreCase("size")){p.sx=clamp(get("X"),.1f,1024);p.sy=clamp(get("Y"),.1f,1024);p.sz=clamp(get("Z"),.1f,1024);}
