...
public final class Net {
    public static final int PORT=53640,DISCOVERY=53641,MAGIC=0x434c414e,VERSION=5;
    public static final class Snapshot {
        public final List<Part> parts=new ArrayList<Part>(); public int you;public float water;public String round="CONNECTING",map="Classic Baseplate";public final List<Actor> actors=new ArrayList<Actor>();public final List<String> chat=new ArrayList<String>();
    }
    public static Snapshot snapshot(World w,int you){synchronized(w){Snapshot s=new Snapshot();s.you=you;s.water=w.water;s.round=w.round;s.map=w.mapName;for(Part part:w.parts)s.parts.add(part.copy());for(Actor a:w.actors.values()){Actor b=new Actor(a.id,a.name,false);b.x=a.x;b.y=a.y;b.z=a.z;b.yaw=a.yaw;b.health=a.health;b.phase=a.phase;b.state=a.state;b.appearance=a.appearance.copy();b.tool=a.tool;s.actors.add(b);}s.chat.addAll(w.chat);return s;}}
    public static final class Host implements Closeable {
        ...
        private void discover(){byte[] data=new byte[64];while(running)try{DatagramPacket p=new DatagramPacket(data,data.length);discovery.receive(p);String q=new String(p.getData(),0,p.getLength(),"UTF-8");if(!q.equals("CLASSIC_LAN_DISCOVER_5"))continue;int count;synchronized(world){count=world.actors.size();}byte[] r=("CLASSIC_LAN_5|"+server.getLocalPort()+"|ReVerfyx Server|"+world.mapName+"|"+count+"|20").getBytes("UTF-8");discovery.send(new DatagramPacket(r,r.length,p.getAddress(),p.getPort()));}catch(IOException ignored){}}
    ...
    public static List<String> search()throws IOException{
        List<String> found=new ArrayList<String>();DatagramSocket s=new DatagramSocket();try{s.setBroadcast(true);s.setSoTimeout(350);byte[] q="CLASSIC_LAN_DISCOVER_5".getBytes("UTF-8");
            ...
            long end=System.currentTimeMillis()+1800;while(System.currentTimeMillis()<end){try{DatagramPacket p=new DatagramPacket(new byte[512],512);s.receive(p);String r=new String(p.getData(),0,p.getLength(),"UTF-8");if(r.startsWith("CLASSIC_LAN_5|")){String v=p.getAddress().getHostAddress()+"|"+r;if(!found.contains(v))found.add(v);}}catch(SocketTimeoutException ignored){}}
        }finally{s.close();}return found;
    }
    private static void writeSnapshot(DataOutputStream o,Snapshot s)throws IOException{o.writeInt(s.parts.size());for(Part p:s.parts){o.writeFloat(p.x);o.writeFloat(p.y);o.writeFloat(p.z);o.writeFloat(p.sx);o.writeFloat(p.sy);o.writeFloat(p.sz);o.writeInt(p.color);o.writeFloat(p.rx);o.writeFloat(p.ry);o.writeFloat(p.rz);o.writeFloat(p.transparency);o.writeBoolean(p.ladder);o.writeByte(p.shape.ordinal());}o.writeFloat(s.water);writeText(o,s.round);writeText(o,s.map);o.writeByte(s.actors.size());for(Actor a:s.actors){o.writeInt(a.id);writeText(o,a.name);o.writeFloat(a.x);o.writeFloat(a.y);o.writeFloat(a.z);o.writeFloat(a.yaw);o.writeFloat(a.health);o.writeFloat(a.phase);o.writeByte(a.state.ordinal());a.appearance.write(o);o.writeByte(a.tool);}o.writeByte(s.chat.size());for(String c:s.chat)writeText(o,c);}
    private static Snapshot readSnapshot(DataInputStream i)throws IOException{Snapshot s=new Snapshot();int pc=i.readInt();if(pc<0)throw new IOException("Invalid part count");for(int j=0;j<pc;j++){Part p=new Part(i.readFloat(),i.readFloat(),i.readFloat(),i.readFloat(),i.readFloat(),i.readFloat(),i.readInt());p.rx=i.readFloat();p.ry=i.readFloat();p.rz=i.readFloat();p.transparency=i.readFloat();p.ladder=i.readBoolean();int shape=i.readUnsignedByte();if(shape>=Part.Shape.values().length)throw new IOException("Invalid shape");p.shape=Part.Shape.values()[shape];s.parts.add(p);}s.water=i.readFloat();s.round=readText(i,64);s.map=readText(i,96);int n=i.readUnsignedByte();if(n>20)throw new IOException("Actor limit");for(int j=0;j<n;j++){Actor a=new Actor(i.readInt(),readText(i,96),false);a.x=i.readFloat();a.y=i.readFloat();a.z=i.readFloat();a.yaw=i.readFloat();a.health=i.readFloat();a.phase=i.readFloat();int state=i.readUnsignedByte();if(state>=Actor.State.values().length)throw new IOException("State");a.state=Actor.State.values()[state];a.appearance=Appearance.read(i);a.tool=i.readUnsignedByte();if(a.tool>2)throw new IOException("Tool");s.actors.add(a);}n=i.readUnsignedByte();if(n>8)throw new IOException("Chat limit");for(int j=0;j<n;j++)s.chat.add(readText(i,1024));return s;}
