...
/** v1 framed binary TCP. Server is authoritative; clients send input, never positions. */
public final class Net {
    public static final int PORT=53640,DISCOVERY=53641,MAGIC=0x434c414e,VERSION=2;
    public static final class Snapshot {
        public int you;public float water;public String round="CONNECTING";public final List<Actor> actors=new ArrayList<Actor>();public final List<String> chat=new ArrayList<String>();
    }
    public static Snapshot snapshot(World w,int you){synchronized(w){Snapshot s=new Snapshot();s.you=you;s.water=w.water;s.round=w.round;for(Actor a:w.actors.values()){Actor b=new Actor(a.id,a.name,false);b.x=a.x;b.y=a.y;b.z=a.z;b.yaw=a.yaw;b.health=a.health;b.phase=a.phase;b.state=a.state;s.actors.add(b);}s.chat.addAll(w.chat);return s;}}
    public static final class Host implements Closeable {
        ...
        private void serve(Socket s){Actor a=null;try{
            ...
            String name=readText(in,24),language=readText(in,8);if(!name.matches("[A-Za-z0-9_]{3,20}"))throw new IOException("Invalid username");
            synchronized(world){for(Actor existing:world.actors.values())if(existing.name.equalsIgnoreCase(name))throw new IOException("Username in use");if(world.actors.size()>=20)throw new IOException("Server full");a=world.add(name,false,language);}
            out.writeInt(MAGIC);out.writeInt(a.id);out.flush();
            while(running){int op=in.readUnsignedByte();if(op!=1&&op!=2)throw new IOException("Invalid opcode");float x=in.readFloat(),z=in.readFloat();boolean jump=in.readBoolean();String msg=readText(in,640);if(op==2)world.resetCharacter(a.id);world.input(a.id,x,z,jump);if(msg.length()>0)world.message(a.id,msg);writeSnapshot(out,snapshot(world,a.id));out.flush();}
        }catch(IOException ignored){}finally{if(a!=null)world.remove(a.id);sockets.remove(s);try{s.close();}catch(IOException ignored){}}}
        private void discover(){byte[] data=new byte[64];while(running)try{DatagramPacket p=new DatagramPacket(data,data.length);discovery.receive(p);String q=new String(p.getData(),0,p.getLength(),"UTF-8");if(!q.equals("CLASSIC_LAN_DISCOVER_2"))continue;int count;synchronized(world){count=world.actors.size();}byte[] r=("CLASSIC_LAN_2|"+server.getLocalPort()+"|ReVerfyx Server|Classic Baseplate|"+count+"|20").getBytes("UTF-8");discovery.send(new DatagramPacket(r,r.length,p.getAddress(),p.getPort()));}catch(IOException ignored){}}
    ...
    public static List<String> search()throws IOException{
        List<String> found=new ArrayList<String>();DatagramSocket s=new DatagramSocket();try{s.setBroadcast(true);s.setSoTimeout(350);byte[] q="CLASSIC_LAN_DISCOVER_2".getBytes("UTF-8");
            ...
            long end=System.currentTimeMillis()+1800;while(System.currentTimeMillis()<end){try{DatagramPacket p=new DatagramPacket(new byte[512],512);s.receive(p);String r=new String(p.getData(),0,p.getLength(),"UTF-8");if(r.startsWith("CLASSIC_LAN_2|")){String v=p.getAddress().getHostAddress()+"|"+r;if(!found.contains(v))found.add(v);}}catch(SocketTimeoutException ignored){}}
        }finally{s.close();}return found;
    }
    private static void writeSnapshot(DataOutputStream o,Snapshot s)throws IOException{o.writeFloat(s.water);writeText(o,s.round);o.writeByte(s.actors.size());for(Actor a:s.actors){o.writeInt(a.id);writeText(o,a.name);o.writeFloat(a.x);o.writeFloat(a.y);o.writeFloat(a.z);o.writeFloat(a.yaw);o.writeFloat(a.health);o.writeFloat(a.phase);o.writeByte(a.state.ordinal());}o.writeByte(s.chat.size());for(String c:s.chat)writeText(o,c);}
    private static Snapshot readSnapshot(DataInputStream i)throws IOException{Snapshot s=new Snapshot();s.water=i.readFloat();s.round=readText(i,64);int n=i.readUnsignedByte();if(n>20)throw new IOException("Actor limit");for(int j=0;j<n;j++){Actor a=new Actor(i.readInt(),readText(i,96),false);a.x=i.readFloat();a.y=i.readFloat();a.z=i.readFloat();a.yaw=i.readFloat();a.health=i.readFloat();a.phase=i.readFloat();int state=i.readUnsignedByte();if(state>=Actor.State.values().length)throw new IOException("State");a.state=Actor.State.values()[state];s.actors.add(a);}n=i.readUnsignedByte();if(n>8)throw new IOException("Chat limit");for(int j=0;j<n;j++)s.chat.add(readText(i,1024));return s;}
