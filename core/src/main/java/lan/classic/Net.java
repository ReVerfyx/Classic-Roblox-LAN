...
public final class Net {
    public static final int PORT=53640,DISCOVERY=53641,MAGIC=0x434c414e,VERSION=1;
    ...
    public static final class Host implements Closeable {
        ...
        private void serve(Socket s){Actor a=null;try{
            ...
            while(running){int op=in.readUnsignedByte();if(op!=1)throw new IOException("Invalid opcode");float x=in.readFloat(),z=in.readFloat();boolean jump=in.readBoolean();String msg=readText(in,640);world.input(a.id,x,z,jump);if(msg.length()>0)world.message(a.id,msg);writeSnapshot(out,snapshot(world,a.id));out.flush();}
        }catch(IOException ignored){}finally{if(a!=null)world.remove(a.id);sockets.remove(s);try{s.close();}catch(IOException ignored){}}}
        private void discover(){byte[] data=new byte[64];while(running)try{DatagramPacket p=new DatagramPacket(data,data.length);discovery.receive(p);String q=new String(p.getData(),0,p.getLength(),"UTF-8");if(!q.equals("CLASSIC_LAN_DISCOVER_1"))continue;int count;synchronized(world){count=world.actors.size();}byte[] r=("CLASSIC_LAN_1|"+server.getLocalPort()+"|ReVerfyx Server|Classic Baseplate|"+count+"|20").getBytes("UTF-8");discovery.send(new DatagramPacket(r,r.length,p.getAddress(),p.getPort()));}catch(IOException ignored){}}
    ...
    public static List<String> search()throws IOException{
        List<String> found=new ArrayList<String>();DatagramSocket s=new DatagramSocket();try{s.setBroadcast(true);s.setSoTimeout(350);byte[] q="CLASSIC_LAN_DISCOVER_1".getBytes("UTF-8");
            ...
            long end=System.currentTimeMillis()+1800;while(System.currentTimeMillis()<end){try{DatagramPacket p=new DatagramPacket(new byte[512],512);s.receive(p);String r=new String(p.getData(),0,p.getLength(),"UTF-8");if(r.startsWith("CLASSIC_LAN_1|")){String v=p.getAddress().getHostAddress()+"|"+r;if(!found.contains(v))found.add(v);}}catch(SocketTimeoutException ignored){}}
